...
const promptTemplate = new PromptTemplate({
  template: `
Você é um assistente especializado em aplicações práticas de AI Agents que responde perguntas com base no contexto fornecido.
...
- Mantenha o foco em aplicações práticas e casos de uso reais de AI Agents.
`.trim(),
...
const askQuestion = (rl: readline.Interface): Promise<string> => {
  return new Promise((resolve) => {
    rl.question('\nFaça sua pergunta sobre aplicações práticas de AI Agents (ou digite "sair" para encerrar): ', (answer) => {
      resolve(answer);
    });
  });
};
...
async function main() {
  ...
  console.log('Bem-vindo ao chat sobre aplicações práticas de AI Agents! Você pode fazer perguntas sobre casos de uso reais, implementações e melhores práticas.');
